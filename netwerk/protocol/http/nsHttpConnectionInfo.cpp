...
nsHttpConnectionInfo::CloneAndAdoptPortAndAlpn(
    uint16_t aPort, happy_eyeballs::ConnectionAttemptProtocols aProtocol) const {
  // See TlsHandshaker::SetupNPNList, "http/1.1" and "h2" are added
  // automatically, so we only need to set "h3".
  nsAutoCString alpnStr(aProtocol == happy_eyeballs::ConnectionAttemptProtocols::H3 ? "h3"_ns
                                                                     : EmptyCString());
  ...
  RefPtr<nsHttpConnectionInfo> clone = new nsHttpConnectionInfo(
      ...
      mEndToEndSSL, aProtocol == happy_eyeballs::ConnectionAttemptProtocols::H3, mWebTransport);
